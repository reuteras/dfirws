...
.EXAMPLE
    ...
    This will run ClamAV and YARA scans in parallel within the same sandbox.
...
# Apply defaults for supply chain security settings if not set by config
if (-not (Test-Path variable:SUPPLY_CHAIN_SECURITY_AUDIT)) { $SUPPLY_CHAIN_SECURITY_AUDIT = $true }
if (-not (Test-Path variable:RUSTSEC_ALLOWLIST))           { $RUSTSEC_ALLOWLIST = @("RUSTSEC-2021-0139", "RUSTSEC-2021-0153", "RUSTSEC-2026-0002") }
...
$npmAllowlistFormatted      = ($NPM_AUDIT_ALLOWLIST | ForEach-Object { "`"$_`"" }) -join ", "
$pipAllowlistFormatted      = ($PIP_AUDIT_ALLOWLIST | ForEach-Object { "`"$_`"" }) -join ", "
$goAllowlistFormatted       = ($GO_VULN_ALLOWLIST   | ForEach-Object { "`"$_`"" }) -join ", "
$profileContent = @"
# Auto-generated by downloadFiles.ps1 - do not edit
`$DFIRWS_EXCLUDE_TOOLS = @($( ($DFIRWS_EXCLUDE_TOOLS | ForEach-Object { "`"$_`"" }) -join ", " ))
`$SUPPLY_CHAIN_SECURITY_AUDIT = `$$($SUPPLY_CHAIN_SECURITY_AUDIT.ToString().ToLower())
`$RUSTSEC_ALLOWLIST  = @($rustsecAllowlistFormatted)
`$NPM_AUDIT_ALLOWLIST = @($npmAllowlistFormatted)
`$PIP_AUDIT_ALLOWLIST = @($pipAllowlistFormatted)
`$GO_VULN_ALLOWLIST   = @($goAllowlistFormatted)
"@
...
if ($ClamScan.IsPresent -and $YaraScan.IsPresent) {
    ...
    .\resources\download\clamscan_yarascan.ps1 | Out-Null
