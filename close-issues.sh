#!/bin/bash
# Script to close GitHub issues for completed tools
# Requires: GitHub CLI (gh)
# Usage: ./close-issues.sh

echo "Closing GitHub issues for completed tools..."
echo ""

# First, create the PR if it doesn't exist
echo "Step 1: Creating PR #127 (if it doesn't exist)..."
gh pr create \
  --title "DFIRWS v2 Complete Migration - All Tools & Infrastructure (437 tools, 100% complete)" \
  --body-file - \
  --head claude/migrate-remaining-tools-v2-011CUpRcJ3piSogFNpkYUv3C \
  --base main << 'EOF'
...
echo ""
echo "✅ All done! Issues closed and comments added."
echo ""
echo "Summary:"
echo "- PR #127 created (if it didn't exist)"
echo "- Issue #120 (QELP) - CLOSED"
echo "- Issue #118 (pdf_object_hashing) - CLOSED"
echo "- Issue #116 (minusone) - CLOSED"
echo "- Issue #115 (OneDriveExplorer) - CLOSED"
echo "- Issue #97 (Windows 11 right-click) - Comment added, KEPT OPEN"
