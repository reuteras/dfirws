There are a couple of tools available in **dfirws** to investigate PDF files. Some of them are:

- **pdf-parser.py**
- **[pdfalyzer](https://github.com/michelcrypt4d4mus/pdfalyzer)**
- **pdfid.py**
- **PDFStreamDumper**
- **[peepdf-3](https://pypi.org/project/peepdf-3/)** (virtualenv name _peepdf3_)
- [[qpdf]]

There is a Jupyter [Notebook](https://github.com/reuteras/dfirws/blob/main/setup/jupyter/pdf.ipynb) available to analyse PDF files.

## Resources

- You can see this local pdf of the article [How to Analyze Malicious PDF Files](../resources/How_to_Analyze_Malicious_PDF_Files.pdf) by [Intezer](https://intezer.com/blog/incident-response/analyze-malicious-pdf-files/).
- [DFIR pdf analysis](../resources/e.200-dfir-pdf-analysis.pdf) by [neolea](https://github.com/neolea/neolea-training-materials)
- [PDF Object Streams](https://isc.sans.edu/forums/diary/PDF+Object+Streams/31430/)
