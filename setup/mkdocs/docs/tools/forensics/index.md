...
| [acquire](acquire.md) | Python | acquire --help | forensics, incident-response, acquisition, disk-forensics | `.tar` |  |
...
| [dissect](dissect.md) | Python | target-query -f users <image> | forensics, incident-response, data-extraction | `.dd`, `.raw`, `.tar` |  |
| [dissect.target](dissect-target.md) | Python | target-shell <image> | forensics, incident-response, artifact-extraction | `.dd`, `.raw`, `.tar`, `.vmdk`, `.E01` |  |
| [flow.record](flow-record.md) | Python | rdump records.rec -w out.csv | forensics, data-processing | `.rec` |  |
| [msticpy](msticpy.md) | Python | import msticpy | threat-intelligence, incident-response, python | `.json`, `.csv` |  |
| [one-extract](one-extract.md) | Git | Python library for extracting objects from OneNote files. | forensics, office, data-extraction |  |  |
| [pathlab](pathlab.md) | Python | import pathlab | forensics, filesystem |  |  |