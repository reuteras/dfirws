...
if ($SUPPLY_CHAIN_SECURITY_AUDIT) {
    ...
    Write-DateLog "Rust: Supply chain security audit disabled - skipping cargo-audit." >> "C:\log\rust.txt"
}
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "A family of small tools: mft2bodyfile, usnjrnl_dump, evtx2bodyfile, evtxanalyze, evtxscan, regdump, cleanhive, pol_export, ipgrep, ts2date, zip2bodyfile. Chain the bodyfile outputs into mactime2 for a unified timeline."
    Usage = "mft2bodyfile <MFT file> > mft.bodyfile (see each tool's --help)"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("msys2")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Export the J stream of the UsnJrnl from the image first (icat from Sleuthkit or MFTECmd can do it). The output includes the reason flags; filter on FILE_CREATE and RENAME_NEW_NAME to spot tool staging and cleanup."
    Usage = "usnjrnl_dump <UsnJrnl J stream> > usn.csv"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("msys2")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Terminal HTTP client for crafting requests and saving them as curl commands or collections; use it from the network sandbox when probing a suspicious endpoint."
    Usage = "cute"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("msys2")
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Fingerprints SSH sessions in a PCAP and infers keystrokes, commands, sudo prompts and file transfers from packet sizes and timing. Works best on captures containing a single interactive session."
    Usage = "sshniff -f capture.pcap"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("msys2")
    Homepage = ""
