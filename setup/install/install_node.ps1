...
Save-NpmMetadata -Package "deobfuscator" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
Save-NpmMetadata -Package "docsify-cli" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
Save-NpmMetadata -Package "jsdom" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
Save-NpmMetadata -Package "box-js" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
Save-NpmMetadata -Package "@marp-team/marp-cli" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
Save-NpmMetadata -Package "opencode-ai" 2>&1 | ForEach-Object{ "$_" } >> "C:\log\npm.txt"
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Emulates JScript and WScript malware and logs dropped files, URLs, shell commands and ActiveX use. Use --loglevel debug when a sample exits early, --no-kill to continue past errors and --timeout to give long sleeps a chance."
    Usage = "box-js <sample.js> --output-dir <dir>"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Cleans output from javascript-obfuscator (string arrays, control flow flattening, dead code). Run js-beautify on the result for readability, then box-js to observe behaviour."
    Usage = "synchrony deobfuscate <file.js>"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Serves a folder of Markdown as a website; handy for reading documentation repositories or your own notes in the sandbox browser."
    Usage = "docsify serve <docs folder>"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Use jsdom in Node scripts to emulate a browser DOM when stepping through malicious JavaScript that expects document and window objects. box-js is the packaged alternative for WScript samples."
    Usage = "node -e `"const {JSDOM} = require('jsdom'); const dom = new JSDOM('<p>hi</p>'); console.log(dom.window.document.body.textContent)`""
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Browser based EVTX viewer for quick visual review and filtering of a few logs. Use hayabusa, chainsaw or EventHawk for bulk analysis. Excluded from the Basic profile."
    Usage = "Start from the desktop shortcut (lumen.ps1) and open EVTX files in the browser."
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tags = @("markdown", "office", "documentation")
    Notes = "A CLI interface for Marp and Marpit based converters. Markdown presentations."
    Tips = "Converts Markdown to slide decks (HTML, PDF, PPTX) for presenting findings. Use --html to allow inline HTML and --theme for custom styling."
    Usage = "marp slides.md -o slides.pdf"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @("nodejs")
    Homepage = ""
