...
# This script runs in a Windows sandbox to scan tools with ClamAV and YARA in parallel.
# YARA scan starts immediately as a background job while ClamAV is being installed.
Write-DateLog "Install ClamAV and run ClamAV and YARA scans in parallel" | Tee-Object -FilePath "C:\log\clamscan.txt" -Append
...
# Start YARA scan as a background job immediately — runs while ClamAV is being installed
$yaraJob = $null
...
# Install ClamAV (YARA is already running in background during this time)
Install-ClamAV
...
$ClamScanLog = "C:\log\clamav-scan.log"
...
Write-DateLog "ClamAV scan complete. Results saved to $ClamScanLog" | Tee-Object -FilePath "C:\log\clamscan.txt" -Append
...
if ($null -ne $yaraJob) {
    ...
    Remove-Job $yaraJob
    Write-DateLog "YARA scan complete. Results saved to $YaraScanLog" | Tee-Object -FilePath "C:\log\clamscan.txt" -Append
}
