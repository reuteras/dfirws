...
Add-Shortcut -SourceLnk "${HOME}\Desktop\dfirws\Forensics\rgeoip.exe (- dissect).lnk" -DestinationPath "${POWERSHELL_EXE}" -WorkingDirectory "${HOME}\Desktop" -Arguments "-NoExit -command rgeoip.exe -h"
...
Copy-Item -Recurse "${GIT_PATH}\AuthLogParser" "${env:ProgramFiles}\"
