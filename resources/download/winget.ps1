...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand. The community edition works offline for manual proxying; use the network sandbox and point a browser at 127.0.0.1:8080 to inspect web traffic from a sample or a suspicious site."
    Usage = "dfirws-install.ps1 -BurpSuite"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand for viewing HTML reports and browser based tools such as CyberChef, LUMEN and hindsight_gui. Keep the sandbox offline when opening untrusted HTML."
    Usage = "dfirws-install.ps1 -Chrome"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Required by tools built for .NET 8. Check installed runtimes with 'dotnet --list-runtimes'."
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Required by the current Eric Zimmerman tools and other .NET 9 applications. Check installed runtimes with 'dotnet --list-runtimes'."
    Usage = "Installed silently during sandbox start; nothing to run."
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Fast viewer for images extracted from disk images, PCAPs and thumbcaches; supports batch conversion and renaming via File -> Batch Conversion and shows EXIF data with the i key."
    Usage = "i_view64.exe <image file>"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand for case notes. Create the vault in the readwrite folder so it survives closing the sandbox. Plugins downloaded by dfirws (dataview, kanban, MITRE ATT&CK vault and others) can be installed offline from the Tools folder."
    Usage = "dfirws-install.ps1 -Obsidian"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Enable with WSDFIR_OHMYPOSH set to Yes in the configuration to get a themed prompt with git and venv information. Cosmetic only."
    Usage = "dfirws-install.ps1 -OhMyPosh"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "PowerShell 7 is the default shell in the sandbox terminal. Windows PowerShell 5.1 is still available as powershell.exe for scripts and modules that need it."
    Usage = "pwsh"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand for SSH and serial connections from the network sandbox; plink and pscp come with it for scripted transfers."
    Usage = "dfirws-install.ps1 -PuTTY"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "qemu-img converts between VMDK, VHDX, QCOW2 and raw so other tools can parse the disk, and 'qemu-img info' shows the format and snapshots. Full system emulation works but is slow inside the sandbox."
    Usage = "qemu-img convert -f vmdk -O raw disk.vmdk disk.raw"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Runtime for Ruby based scripts and tools. Gems must be installed from the network sandbox since the analysis sandbox is offline."
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand to play audio and video recovered from evidence or screen recordings; it handles partial and damaged files better than most players."
    Usage = "dfirws-install.ps1 -VLC"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Run 'vt init' or set VTCLI_APIKEY first and use the network sandbox. Look up by hash with 'vt file <hash>' to avoid uploading, and remember that uploads share the sample with the VirusTotal community."
    Usage = "vt file <sha256 or file path>"
    SampleCommands = @(
        "vt file sample.exe",
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand to diff files or whole folders, for example a clean and a suspect copy of a system directory or two versions of a script."
    Usage = "dfirws-install.ps1 -WinMerge"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
if (Test-ToolIncluded -ToolName "OpenVPN") {
    ...
    $TOOL_DEFINITIONS += @{
        ...
        Tips = "Only useful in the network enabled sandbox; use it to reach an isolated lab network or a customer VPN."
        Usage = "Install on demand and import a .ovpn profile in the OpenVPN GUI."
        SampleCommands = @()
        SampleFiles = @()
        Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand to plot coordinates from EXIF data or GeoIP results; map tiles need the network sandbox."
    Usage = "dfirws-install.ps1 -GoogleEarth"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Only useful in the network enabled sandbox for reaching a lab network or VPN endpoint."
    Usage = "Install on demand and import a tunnel configuration in the WireGuard GUI."
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand. tshark is added to PATH too so you can script extraction (tshark -r file.pcap -Y http -T fields -e http.host). Add the PacketCircle plugin for a traffic matrix view."
    Usage = "dfirws-install.ps1 -Wireshark"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Joins the sandbox to a tailnet for transferring evidence securely between machines; only works in the network enabled sandbox."
    Usage = "Install on demand and run 'tailscale up' in the network sandbox."
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand as an alternative browser for HTML reports and web based tools. Keep the sandbox offline when opening untrusted HTML."
    Usage = "dfirws-install.ps1 -Firefox"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand for reading PDF reports and documentation. Do not open suspicious PDFs in it; use pdfid, pdf-parser, peepdf or pdfalyzer for those."
    Usage = "dfirws-install.ps1 -FoxitReader"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Open crash dumps and full memory dumps in the modern WinDbg; load the NetExt extension with .load for managed heap analysis. Symbol downloads need the network sandbox or a local symbol store."
    Usage = "windbgx <dump file>"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand as a terminal editor. The pynvim Python package is included so Python based plugins work."
    Usage = "dfirws-install.ps1 -Neovim"
    SampleCommands = @(
        "nvim notes.md"
    )
    SampleFiles = @()
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Converts Windows network traces captured with 'netsh trace' or pktmon to PCAPNG for Wireshark and NetworkMiner. Only the packet capture provider events are converted."
    Usage = "etl2pcapng trace.etl trace.pcapng"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
