...
if (! (Test-Path -Path $ROOT_PATH\mount\venv )) {
    New-Item -ItemType Directory -Force -Path $ROOT_PATH\mount\venv > $null
}
...
foreach ($venv in $venvs) {
    if (! (Test-Path -Path $ROOT_PATH\mount\venv\$venv )) {
        New-Item -ItemType Directory -Force -Path $ROOT_PATH\mount\venv\$venv > $null
    }
}
...
if (Test-Path -Path $ROOT_PATH\mount\venv\default\done ) {
    Remove-Item $ROOT_PATH\mount\venv\default\done > $null
}
