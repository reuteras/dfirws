...
Write-DateLog "Downloading ${manufUrl}"
...
if (-not "${IPINFO_API_KEY}") {
    Write-DateLog "Please set the IPINFO_API_KEY variable in config.ps1 if you want to download IPinfo databases"
...
if (-not "${MAXMIND_LICENSE_KEY}") {
    ...
    Write-DateLog "Copying Maxmind GeoLite2 databases to maxmind_current"
...
Write-DateLog "Downloading Geolocus mmdb file"
...
Write-DateLog "Downloading $suricataUrl"
...
Write-DateLog "Downloading $snortUrl"
...
Write-DateLog "Updating git repositories."
...
# Yara rules
Write-DateLog "Downloading Yara rules"
...
$TOOL_DEFINITIONS += @{
    ...
    Verify = @()
    FileExtensions = @()
    Tags = @("blocklist", "network", "threat-intelligence")
...
$TOOL_DEFINITIONS += @{
    ...
    Tags = @("ids", "detection-rules", "network")
...
$TOOL_DEFINITIONS += @{
    ...
    Tags = @("vulnerability", "threat-intelligence")
...
$TOOL_DEFINITIONS += @{
    ...
    Verify = @()
    FileExtensions = @()
    Tags = @("threat-intelligence", "network", "blocklist")
