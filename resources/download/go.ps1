...
if (Test-Path -Path "${ROOT_PATH}\log\.metadata\go" ) {
    New-Item -ItemType Directory -Force -Path "${ROOT_PATH}\mount\Tools\.metadata\go" | Out-Null
    Copy-Item "${ROOT_PATH}\log\.metadata\go\*.json" "${ROOT_PATH}\mount\Tools\.metadata\go\" -Force
    Remove-Item -Recurse -Force "${ROOT_PATH}\log\.metadata\go" 2>&1 | Out-Null
}