...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Install on demand. Mounts E01, raw, VHD, VMDK and ISO images read-only as drive letters so other tools can parse the file system. Use the write cache option only when you need to boot or modify a copy."
    Usage = "dfirws-install.ps1 -OSFMount"
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
    Homepage = ""
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Community VQL artifacts that extend Velociraptor. Review the VQL before running an artifact on endpoints; many are collection heavy."
    Usage = "Import the artifact zip into Velociraptor (Server Artifacts -> Upload)."
    SampleCommands = @()
    SampleFiles = @(
        "N/A"
    )
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Installed for tools that require the Microsoft build of OpenJDK. Amazon Corretto is the default java on PATH."
    Usage = "Run with the full path when a tool requires this JDK."
    SampleCommands = @()
    SampleFiles = @()
    Dependencies = @()
    PythonVersion = ""
...
if (Test-ToolIncluded -ToolName "Neo4j") {
    $NeoVersionString = Get-DownloadUrlFromPage -Url "https://neo4j.com/deployment-center/#community" -RegEx 'release=(2[^&]+)&flavour=winzip'
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Extracts files, images, credentials, sessions and host details from PCAPs passively. TLS traffic must be decrypted first, for example with PolarProxy. The free version has no command line."
    Usage = "GUI: start from the desktop shortcut and open a PCAP file."
    SampleCommands = @()
    SampleFiles = @(
        "N/A"
    )
...
$TOOL_DEFINITIONS += @{
    ...
    Tips = "Transparent TLS proxy that writes decrypted traffic as PCAP for Wireshark and NetworkMiner. Install its root CA in the client you are inspecting and use the network enabled sandbox when the client needs internet."
    Usage = "PolarProxy.exe -p 10443,80,443 -w C:\Users\WDAGUtilityAccount\Desktop\readwrite\decrypted.pcap"
    SampleCommands = @()
    SampleFiles = @(
        "N/A"
    )
