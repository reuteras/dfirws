...
├── dfirws-config.ps1.template            # Template for API keys (GitHub, MaxMind)
│
├── resources/
...
| generate_msys2.wsb | install_msys2.ps1 | MSYS2 environment + r2ai plugin + pycdc (built from git, static) | msys2 gcc, cmake, pkg-config |
...
See [docs/categories-and-tags.md](docs/categories-and-tags.md) for the full canonical category list.
...
See [docs/categories-and-tags.md](docs/categories-and-tags.md) for the full canonical tag list (organized by analysis domain, file format, technique, tool type, platform, security area, and enrichment source).

### SourceType Field

Automatically set by `New-CreateToolFiles` from the `-Source` parameter. Maps to display labels in the wiki:

See [docs/categories-and-tags.md](docs/categories-and-tags.md) for the full Source → Wiki Label table.
