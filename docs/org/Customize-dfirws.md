...
- Change installed programs in *local\config.txt*. You can also change configuration file for [Sysmon](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon).
- Change shortcuts on the desktop or run your own **PowerShell** command on startup by creating *local\customize.ps1* by first copying *local\defaults\customize.ps1*.
